...
def _process_single_sheet(sheet):
    merged_cells_info = {}
    for merged_range in list(sheet.merged_cells.ranges):
        min_col, min_row, max_col, max_row = openpyxl.utils.cell.range_boundaries(merged_range.coord)
        cell_value = sheet.cell(row=min_row, column=min_col).value

        merged_cells_info[merged_range.coord] = {
            'value': cell_value,
            'min_row': min_row,
            'max_row': max_row,
            'min_col': min_col,
    ...
    for _, info in merged_cells_info.items():
        for row_index in range(info['min_row'], info['max_row'] + 1):
            for col_index in range(info['min_col'], info['max_col'] + 1):
                sheet.cell(row=row_index, column=col_index).value = info['value']
    tables_data = []
    processed_cells = set()

    max_row = sheet.max_row
    max_col = sheet.max_col if hasattr(sheet, 'max_col') else sheet.max_column
    ...
    for row_idx in range(1, max_row + 1):
        for col_idx in range(1, max_col + 1):
            ...
            if cell.value is not None and str(cell.value).strip() != '':
                table_min_row = row_idx
                table_min_col = col_idx

                table_max_col = table_min_col
                while sheet.cell(row=table_min_row, column=table_max_col + 1).value is not None and \
                        str(sheet.cell(row=table_min_row, column=table_max_col + 1).value).strip() != '':
                    table_max_col += 1
                ...
                while True:
                    ...
                    for c in range(table_min_col, table_max_col + 1):
                        cell_value = sheet.cell(row=table_max_row + 1, column=c).value
                ...
                for r in range(table_min_row, table_max_row + 1):
                    row_data = [sheet.cell(row=r, column=c).value for c in range(table_min_col, table_max_col + 1)]
                    table_data.append(row_data)
                    for c in range(table_min_col, table_max_col + 1):
    ...
    for table in tables_data:
        if not table or not any(any(cell is not None and str(cell).strip() != '' for cell in row) for row in table):
            continue
        markdown_str = []
        header = table[0]
        header_str = ' | '.join(str(h) if h is not None else '' for h in header)
        markdown_str.append(f'| {header_str} |')

        separator = '---'
        separator_str = ' | '.join([separator] * len(header))
        markdown_str.append(f'| {separator_str} |')

        for row_data in table[1:]:
            row_data_str = ' | '.join(str(cell) if cell is not None else '' for cell in row_data)
            markdown_str.append(f'| {row_data_str} |')
...
def get_md3(wb):
    ...
    try:
        ...
        for sheet_name in workbook.sheetnames:
            ...
            markdown_tables = _process_single_sheet(sheet)
            
            if markdown_tables:
                all_output.append(f"# --- Sheet: {sheet_name} ---")
                for i, table_str in enumerate(markdown_tables):
                    all_output.append(f"\n--- Table {i+1} ---")
        ...
        return f"error in parse file: {e}"



@router.post("/parse_xlsx/")
async def upload_file(
        file: UploadFile = File(...),
        type: int = Query(1, description="xlsx解析模式")
